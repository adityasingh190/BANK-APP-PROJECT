import axios from 'axios';
import React, { useState } from "react";
import {Navbar, Nav} from 'react-bootstrap';
import {Link,useLocation} from 'react-router-dom';

import {Jumbotron,Dropdown} from 'react-bootstrap';
import {Card, Form, Button, Col} from 'react-bootstrap'
export default function Update1(props)
{
    const location=useLocation();
const [showMessage, setShowMessage] = useState(false);
const [accNo, setaccNo] = useState(location.state.accNo);
const[accNo1,setaccNo1]=useState("");
const [custName, setcustName] = useState("");
const [customerId, setcustomerId] = useState("");
// const [IFSCcode, setIFSCcode] = useState("");
const [ifscCode, setifscCode] = useState("");
const [balance, setbalance] = useState("");
const [password, setpassword] = useState("");
  const [formerrors, setFormErrors] = useState({});

const USER_REGEX = /^\d{9,18}$/;
const PWD_REGEX = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%]).{8,24}$/;
const IFSC_REGEX = /[A-Z|a-z]{4}[0][a-zA-Z0-9]{6}$/;

const submitupdate=(event)=> {
    event.preventDefault();
    
    const update = {
         accNo: location.state.accNo,
         accNo1:accNo1,
        custName: custName,
        customerId:customerId,
        IFSC_code:ifscCode,
        password:password
        
    };
if(!(/[A-Z|a-z]{4}[0][a-zA-Z0-9]{6}$/.test(ifscCode)))
{
alert("IFSC Code is invalid");
}
else if(!(/^\d{9,18}$/.test(accNo1)))
alert("Account number is invalid");
else if(!PWD_REGEX.test(password))
alert("Password is in wrong pattern");

else{
    axios.put("http://localhost:8081//Update2", {}, {params:update}, { headers: {
    'Access-Control-Allow-Origin': '*', 
    'Access-Control-Allow-Methods': 'POST, PATCH, PUT, DELETE, OPTIONS',
    'Access-Control-Allow-Headers': 'Origin, Content-Type, X-Auth-Token',
    'Access-Control-Allow-Credentials': true}},
    {withCredentials: true},
    {crossorigin: true})
    .then(response => {
        console.log(response);
        if (response.data === ""){
            alert("Update did not happen as entered account number not found");
        }else{document.getElementById("d").innerHTML="Details updated"}
    }).catch((error=>console.log(error.response.request._response)));
}}

return (
    <>
    <div >
    <Navbar className="bg-dark" variant="dark" style={{backgroundColor:"black"}}>
        <Link to={""} className="navbar-brand">
            <img
                alt=""
                src="data:image/png;base64,"
                width="30"
                height="30"
                className="d-inline-block align-top"
            />{' '}
        </Link>
        <Nav className="mr-auto" >
        <Link to={"/NavigationBar1"} state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Home</Link>
        
         
            <Link to="/Deposit1" state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Deposit</Link>
            <Link to="/Withdraw1" state={{accNo:accNo}} style={{color:"white"}}className="nav-link">Withdraw</Link>
            <Link to='/Update1'  state={{accNo:accNo}} style={{color:"white"}}className="nav-link">Update</Link>
            <Link to="/Transfer1" state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Transfer</Link>
            <Link to="/Balance" state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Balance</Link>
            <Link to="/Transferdetails1"  state={{accNo:accNo}} style={{color:"white"}}className="nav-link">Transfer Details</Link>
            <Link to="/DepositDetails1"state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Deposit Details</Link>
            <Link to="/WithrawDetails1" state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Withraw  Details</Link>
            <Link to="/Shopping" state={{accNo:accNo}} style={{color:"white"}} className="nav-link">Shopping</Link>
            <Dropdown className="dropdown-groove">
               <Dropdown.Toggle variant="link" style={{color:"white",textDecoration:"none"}}>
    Log out
  </Dropdown.Toggle  >
  <Dropdown.Menu>
    
      <Dropdown.Item style={{color:"black"}} as={Link} to="/Login">Log out</Dropdown.Item>
    
	
      <Dropdown.Item style={{color:"black"}} as={Link} to="/Register" >Log in as Admin</Dropdown.Item>
    
  </Dropdown.Menu>
</Dropdown>
        </Nav>
      </Navbar>
      <div style={{backgroundSize:"cover",backgroundImage:"url(`${kam.jpg}`)"}}>
     <Jumbotron className=" text-dark" style={{backgroundColor:"#f0e68c",height:320,opacity:0.5 ,paddingBottom:0,marginBottom:20,backgroundSize:30,width:1533 ,backgroundSize:"cover" ,backgroundImage:"url('data:image/jpeg;base64,')"}}>
         <div style={{marginLeft:20}}>
    <h1 
style={{color:"rgb(176,0,0)",marginLeft:440,display:"table",backgroundColor:"black"}} > WELCOME TO BANKING</h1>
          <h2 style={{color:"black",marginLeft:590}} >
          UPDATE FORM
          </h2>
          </div>
    </Jumbotron>
    </div >
    </div>
    <div >
    {/* <div style={{backgroundRepeat:"no-repeat",backgroundSize:"cover",backgroundImage:"url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQVr19m816je6tE6b8rusGLwjzq2bcVsUhprg&usqp=CAU')"}}> */}
    <div>
    <Card style={{alignItems:"center",justifyContent:"center",marginLeft:450,width:650,backgroundSize:"cover",backgroundImage:"url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQbdUmlrAvwDMX2KFXEq6QKnBd-r5BRaFK5bQ&usqp=CAU')"}} className={"border border-dark  text-white"}>
        <Card.Header style={{}} className={"text-white bg-dark"}>ENTER REQUIRED DETAILS</Card.Header>
            <Form onSubmit={submitupdate} id="depositFormId" style={{alignItems:"center",justifyContent:"center"}}>
                <Card.Body style={{alignItems:"center",justifyContent:"center"}}>
                      <Form.Row >
                          <Form.Group as={Col} controlId="formGridTitle" >
                          <Form.Label style={{marginLeft:140}}className='text-dark'>New Account number</Form.Label>
							  	  <Form.Control required style={{width:500}}
							  	  	type="text" name="accNo1"
							  	  	// value={this.state.accNo1}
							  	  	onChange={event => setaccNo1(event.target.value)}
							  	  	className={"bg-dark text-white"}
							  	  	placeholder="Enter new Account number" />
							  	  <Form.Label style={{marginLeft:160}}className='text-dark'>Customer Name</Form.Label>
							  	  <Form.Control required style={{width:500}}
							  	  	type="text" name="custName"
							  	  	// value={this.state.custName}
							  	  	onChange={event => setcustName(event.target.value)}
							  	  	className={"bg-dark text-white"}
							  	  	placeholder="Enter Customer Name" />
                                     <Form.Label style={{marginLeft:180}} className='text-dark'>Customer Id</Form.Label>
							  	  <Form.Control required style={{width:500}} maxLength="6" minLength="6"
							  	  	type="text" name="customerId"
							  	  	// value={this.state.customerId}
							  	  	onChange={event => setcustomerId(event.target.value)}
							  	  	className={"bg-dark text-white"}
							  	  	placeholder="Enter Customer Id" />
                                     <Form.Label style={{marginLeft:190}}className='text-dark'>IFSC Code</Form.Label>
							  	  <Form.Control required style={{width:500}}
							  	  	type="text" name="IFSC_code"
							  	  	// value={this.state.IFSC_code}
							  	  	onChange={event => setifscCode(event.target.value)}
							  	  	className={"bg-dark text-white"}
							  	  	placeholder="Enter IFSC code" />


<Form.Label style={{marginLeft:190}} className='text-dark'>Password</Form.Label>
                                   <Form.Control required style={{width:500}}
							  	  	type="text" name="password"
							  	  	// value={this.state.password}
							  	  	onChange={event => setpassword(event.target.value)}
							  	  	className={"bg-dark text-white"}
							  	  	placeholder="Enter password" />
							  	</Form.Group>
							  </Form.Row>
						
						</Card.Body>
						<Card.Footer  style={{"textAlign":"right"}}>
							<Button style={{marginRight:15}}size="sm" variant="success" type="submit">
							  Update
							</Button>
                            <p style={{marginLeft:5,color:"black"}}id="d"></p>
						</Card.Footer>
					</Form>
			</Card>
            </div>
            </div>
			</>
		);

}