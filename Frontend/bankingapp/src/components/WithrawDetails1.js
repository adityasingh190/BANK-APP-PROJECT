...
export default function WithrawDetails1 (props) {
...
console.log(f);
		axios.get("http://localhost:8081//displayAllWithrawals1", {params:{accNo:accNo}}, { headers: {
...
style={{color:"rgb(176,0,0)",marginLeft:440,display:"table",backgroundColor:"black"}} > WELCOME TO BANKING</h1>
				  <h2 style={{color:"black",marginLeft:530}} >
				  WITHDRAW DETAILS
				  </h2>
				  </div>
			</Jumbotron>
				...
				<Card.Header style={{}} className={"text-white bg-dark"}>ENTER REQUIRED DETAILS</Card.Header>
				    	...
				    	<thead>
					  	...
					  	<tbody>
						  {
						  	users.length === 0 ?
						  	<tr align="center">
						    	<td colSpan="6">No Withrawals are there to be shown.</td>
						    </tr> : 
						    users.map((user) => (
						    <tr key={user._id}>
					    		<td>{user.accNo}</td>
					    		<td>{user.withraw_amount}</td>
