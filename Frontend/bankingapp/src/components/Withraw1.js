...
export default function Withraw1 (props) {
    const location=useLocation();
	const [withraw_amount, setwithraw_amount] = useState("");
...
console.log(location.state.accNo);
console.log(location);
		const withraw = {
			accNo:location.state.accNo,
			withraw_amount: withraw_amount
		};
		

		axios.put("http://localhost:8081//withraw", {}, {params: withraw}, { headers: {
...
style={{color:"rgb(176,0,0)",marginLeft:440,display:"table",backgroundColor:"black"}} > WELCOME TO BANKING</h1>
				  <h2 style={{color:"black",marginLeft:550}} >
				  WITHRAWAL FORM
				  </h2>
				  </div>
			</Jumbotron>
			...
			<Card style={{alignItems:"center",justifyContent:"center",marginLeft:390,width:650,backgroundSize:"cover",backgroundImage:"url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQbdUmlrAvwDMX2KFXEq6QKnBd-r5BRaFK5bQ&usqp=CAU')"}} className={"border border-dark  text-white"}>
				<Card.Header style={{}} className={"text-white bg-dark"}>ENTER REQUIRED DETAILS</Card.Header>
					<Form onSubmit={submitDeposit} id="depositFormId">
						<Card.Body>
							  <Form.Row>
							  	<Form.Group as={Col} controlId="formGridTitle">
							  	  ...
							  	  <Form.Label style={{marginLeft:100,color:"black"}}><strong>Amount</strong></Form.Label>
							  	  <Form.Control required
							  	  	type="number" name="withraw_amount"
							  	  	
							  	  	onChange={event=>setwithraw_amount(event.target.value)}
						...
						<Card.Footer style={{"textAlign":"right"}} >
							<Button style={{marginRight:100}} size="sm" variant="success" type="submit">
							  Submit
							</Button>
						</Card.Footer>
					</Form>
