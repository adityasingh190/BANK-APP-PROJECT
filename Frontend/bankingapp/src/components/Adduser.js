...
//   };
  ...
  const validate = () => {
    ...
    let errors = {};
const u=USER_REGEX.test(Number.parseInt(accNo));
const i=IFSC_REGEX.test(ifscCode);
// const i=IFSC_REGEX.test(ifscCode);
    //name field
    if (accNo===" ") {
      errors.accNo = "Full account Number is required";
    }
	else if (!u) {
		errors.accNo = "Account number is invalid";
	  }
 ...
 if (customerId===" ") {
      errors.customerId = "Full Id is required";
    } 
	

//  if (IFSCcode===" ") {
//       errors.IFSCcode = "IFSC_code is required";}
if (ifscCode===" ") {
	errors.ifscCode = "IFSC_code is required";}
     else if (!i) {
 ...
 if (password===" ") {
      ...
      errors.password = "password is invalid.    8 to 24 characters. Must include  Letters, numbers, underscores, hyphens allowed.";
    }
	if (balance===" ") {
		errors.balance = "balance is required";
	  } else if (balance<2500) {
		errors.balance = "Enter balance more than 2500";
	  }
...
// 	axios.put("http://localhost:8081//addUser",{},{params:param1}, { headers: {
...
// }
// else
// alert("Please provide all the fields as required in correct format");
}
...
style={{color:"rgb(176,0,0)",marginLeft:440,display:"table",backgroundColor:"black"}} > WELCOME TO BANKING</h1>
    ...
    <Card style={{alignItems:"center",justifyContent:"center",marginLeft:420,width:650,backgroundSize:"cover",backgroundImage:"url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQbdUmlrAvwDMX2KFXEq6QKnBd-r5BRaFK5bQ&usqp=CAU')"}}className={"border border-dark  text-white"}>
				<Card.Header className="text-white bg-dark">ENTER REQUIRED DETAILS</Card.Header>
					<Form onSubmit={handleSubmit} id="bookFormId">
						<Card.Body>
							  <Form.Row>
							  	<Form.Group as={Col} controlId="formGridTitle">
								
                                      	  <Form.Label className="text-dark" style={{marginLeft:56}}>Account number</Form.Label>
									...
									{formerrors.accNo && (
                    <p className="text-danger">{formerrors.accNo}</p>
                  )}
							  	  <Form.Label className="text-dark"  style={{marginLeft:56}}>Customer Name</Form.Label>
							  	  <Form.Control required
									...
									{formerrors.custName && (
                    <p className="text-danger">{formerrors.custName}</p>
                  )}
                                     <Form.Label className="text-dark" style={{marginLeft:66}}>Customer Id</Form.Label>
							  	  <Form.Control required
                    ...
                    <p className="text-danger">{formerrors.customerId}</p>
                                     ...
                                     <Form.Label className="text-dark"  style={{marginLeft:86}}>IFSC Code</Form.Label>
							  	  <Form.Control required
				   ...
				   <Form.Label className="text-dark"  style={{marginLeft:86}}>Balance</Form.Label>
                                   <Form.Control required
                    ...
                    <p className="text-danger">{formerrors.balance}</p>
									   ...
									   <Form.Label className="text-dark"  style={{marginLeft:86}}>Password</Form.Label>
                                   <Form.Control required
